...
(function($){
	...
	$.fn.blogFeed = function(options) {
		...
		// Defaults, extended by any options that were provided
		var settings = $.extend( {
			'maxWords'		: 55,
			'feedPath'		: 'blog',
			'feedQuantity'	: 3,
			'feedImage'		: 0 // 0 (No Image) | 1 (Image)
		}, options);
		
		var imgBox = "";
		
		function getImage(entryContent){
			//Check for image - based on blog.uvm.edu rss, where images are included in entry.content (Not in MediaGroup)
